...
CREATE OR REPLACE TEMP TABLE supp_table2_base AS
SELECT
    CASE
...
continuous_summary_supp2 <- dbGetQuery(con, "
...
categorical_summary_supp2 <- dbGetQuery(con, "
...
# Format summaries into table bodies

continuous_table_supp2 <- continuous_summary_supp2 %>%
  mutate(
    stat = paste0(
...
categorical_table_supp2 <- categorical_summary_supp2 %>%
  mutate(
    stat = paste0(
...
supp_table2_body_long <- bind_rows(
  continuous_table_supp2,
  categorical_table_supp2
)

# Helper function to build each table

make_supp_table2_gt <- function(table_data, funding_value, table_title) {
...
tbl_2a_gt <- make_supp_table2_gt(
  table_data = supp_table2_body_long,
  funding_value = "NIH-funded",
  table_title = "Table 2a: Characteristics of NIH-funded Papers According to Year"
)

tbl_2b_gt <- make_supp_table2_gt(
  table_data = supp_table2_body_long,
  funding_value = "Not NIH-funded",
  table_title = "Table 2b: Characteristics of Non-NIH-funded Papers According to Year"
)

tbl_2a_gt
tbl_2b_gt

# Combine and save supplementary table 2

supplementary_table_2 <- gt_group(
  tbl_2a_gt,
  tbl_2b_gt
)
...
gtsave(
  ...
  "Supplementary Table 2.html"
)

# Save summary data

arrow::write_parquet(
  supp_table2_body_long,
  "supplementary_table_2_summary_long.parquet"
)
