...
## Assess missingness on atypical pairs
...
table_assess_missingness <- dbGetQuery(con, "
  SELECT
      year,
      CASE
          WHEN funding_NIH IS NOT NULL
               AND TRIM(CAST(funding_NIH AS VARCHAR)) <> ''
          THEN 'NIH'
          ELSE 'Not NIH'
      END AS funding,
      COUNT(*) AS total_n,
      SUM(CASE WHEN Atyp_10pct_Z IS NULL THEN 1 ELSE 0 END) AS number_missing,
      100.0 * SUM(CASE WHEN Atyp_10pct_Z IS NULL THEN 1 ELSE 0 END) / COUNT(*) AS percent_missing
  FROM read_parquet('opa_sciscinet_6_18_26_pmid_unique.parquet')
  WHERE is_research_article = TRUE
    AND (
      COALESCE(human, 0) > 0
      OR COALESCE(animal, 0) > 0
      OR COALESCE(molecular_cellular, 0) > 0
    )
  GROUP BY
      year,
      CASE
...
# Identify NIH projects from ExPORTER (data at https://reporter.nih.gov/exporter)
